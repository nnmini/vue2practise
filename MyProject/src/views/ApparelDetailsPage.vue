<template>
  <div>
    <div class="button-container" justify="end">
      ...
      <v-btn color="primary" @click="addColumn" class="left-space"
        >Add Color</v-btn
      >
    </div>
...
export default {
  ...
  methods: {
    ...
    async fetchBillOfMaterials() {
      try {
        ...
        console.log(
          "billOfMaterials:",
          JSON.stringify(this.billOfMaterials, 0, 4)
        );
      } catch (error) {
  ...
  addColumn() {
    // Prompt the user for the column name, you can use a dialog or a simple prompt
    const columnName = prompt("Enter Color Name:");
    if (columnName) {
      // Add the new column definition to columnDefs
      this.columnDefs.push({
        headerName: columnName,
        field: "color_"+columnName.toLowerCase(), // Use lowercase version of the column name as the field
